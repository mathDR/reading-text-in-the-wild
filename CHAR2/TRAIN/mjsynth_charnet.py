...
class MJSYNTH_CHARNET():
    ...
    def __init__(self, which_set, numExamples, numUsedExamples=0):
        ''' Generate a batch of samples for use in charnet.
        Inputs:
           - which_set: training, test or validation set
           - numExamples: the number of desired samples
           - numUsedExamples: the number of samples previously generated
        Returns:
           samples and labels for use in charnet
        '''
        ...
        self.numUsedExamples = numUsedExamples
        ...
        self.datapath = 'FOLDER TO WHERE SYNTH DATA (folder 90kDICT32px/ resides)'
        self.loadData()
    ...
    def findExamples(self):
        with open(self.datapath + self.fileToLoadFrom) as f:
            sampleCounter = 0
            for line in f:
                if sampleCounter < self.numUsedExamples:
                    sampleCounter += 1
                    continue
